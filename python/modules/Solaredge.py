def Monit_SE():
    ...
    for i in range(len(locais)):
        ...
        id_locais = (locais[i]['id'])
        # Busca do ID, usando da variável i para
        # percorrer todo o dicionário e alocando
        # a informação da chave 'id' na variável
        # id_locais

        nome_cliente = (locais[i]['name'])
        # Busca do Nome do Cliente, para
        # implementação de um banco de dados.
        # Mesmo método da variável acima

        energia = s.get_overview(id_locais)
        # Criação da variável 'energia' a qual
        ...
        energia_dia = (energia['overview']['lastDayData']['energy'])
        # Há o escalonamento, buscando
        # os dados de geração de energia
        # em um dia até o momento da captura
        # de dados.
        dados = {}

        dados = id_locais, nome_cliente, energia_dia
        # A inserção dos dados buscados acima. Utilizando do ID
